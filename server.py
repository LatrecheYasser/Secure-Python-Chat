...
print('the server is on ')
  ...
  # 3) wating for clinets to connect :
...
while counter < 2:
    ...
    counter +=1
    print("a client asked for a connecton , adresse IP %s, port %s" % (adresse[0], adresse[1]))
    clients.append([connexion,adresse])

class worker(threading.Thread):
    def __init__(self,client_from,client_to):
        threading.Thread.__init__(self)	     # réf. du socket de connexion
        self.client_from = client_from[0]
        self.client_from_add = client_from[1]
        self.client_to = client_to[0]
        self.client_to_add = client_to[1]
    def run(self):
        #msgClient = self.client_from.recv(2048).decode("Utf8")
        #print('the keys sent from ',self.client_from_add,'to ',self.client_to_add,'is ',msgClient)
        while 1:
            msgClient = self.client_from.recv(1024).decode("Utf8")
